    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 sm:py-12">
      <section className="mx-auto w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <form onSubmit={submit} className="space-y-5 p-5 sm:p-8" autoComplete="off">
          ...
          <div className="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-slate-500">{formRequest?.expires_at ? `This private link expires ${displayDate(formRequest.expires_at)}.` : 'This form is available only from your private email link.'}</p><button disabled={submitting || !form.authorized} className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-700 px-5 py-2.5 text-sm font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-60">{submitting && <Loader2 className="h-4 w-4 animate-spin" />}{submitting ? 'Submitting securely…' : 'Submit securely'}</button></div>
        </form>
      </section>
    </main>
  );
