...
export default function W9Form() {
  ...
  useEffect(() => {
    ...
    getPublicW9(token)
      .then((data) => {
        ...
        setRequest(data);
        if (data.signer_name) {
          setValues((current) => ({ ...current, typed_name: data.signer_name }));
        }
  ...
  async function submit(event) {
    ...
    setSubmitting(true);
    try {
      await submitPublicW9(token, { ...values, signature });
  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-800">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 md:py-10">
        ...
        <section className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-950 flex gap-3"><ShieldCheck className="mt-0.5 shrink-0 w-5 h-5" /><p>Enter the information exactly as it should appear on Form W-9. Your taxpayer identification number is encrypted before it is stored and is not sent by email or displayed in a LegalFlow confirmation.</p></section>
        ...
        <form onSubmit={submit} className="mt-5 space-y-5" autoComplete="off">
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-7">
            ...
            <div className="mt-5 grid gap-4">
              <label className="block"><span className="block text-sm font-semibold text-slate-700">Name (as shown on your income tax return)</span><input required value={values.legal_name} onChange={(e) => update('legal_name', e.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" /></label>
