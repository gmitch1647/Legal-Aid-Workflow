...
export default function W9Requests() {
  ...
  async function notifySigner(request) {
    setError('');
    setSuccess('');
    setSending(true);
    try {
      await notifyW9Signer(request.id);
      setSuccess(`A secure W-9 signing notification was sent to ${request.signer_name}.`);
      await load();
    } catch (err) {
      setError(err.message || 'Unable to send the W-9 signing notification.');
    } finally {
      setSending(false);
    }
  }

  async function requestCorrectedSignature(request) {
    const confirmed = window.confirm(
      `Send ${request.signer_name} a corrected W-9 signature request? The original completed W-9 will be preserved. The new form will retain the prior encrypted taxpayer information, but the client must sign again.`
    );
    if (!confirmed) return;
    setError('');
    setSuccess('');
    setSending(true);
    try {
      const result = await createCorrectedW9SignatureRequest(request.id);
      setSuccess(result.notification_sent
        ? `A corrected W-9 signature request was sent to ${request.signer_name}.`
        : `A corrected W-9 signature request was created for ${request.signer_name}; use Notify Client if the email needs to be resent.`);
      await load();
    } catch (err) {
      setError(err.message || 'Unable to create the corrected W-9 signature request.');
    } finally {
      setSending(false);
    }
  }
