...
export default function ReferralAttorneyWorkspaces() {
  ...
  async function submit(event) {
    ...
    if (!form.full_name.trim() || !form.email.trim() || !form.assigned_attorney_id) {
      setError('Enter the referral partner’s name and email, then choose the LegalFlow attorney assigned to work their submissions.');
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <section className="rounded-2xl bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 px-6 py-7 text-white shadow-lg sm:px-8">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-4 text-2xl font-bold sm:text-3xl">Referral Partner Portals</h1>
            <p className="mt-3 text-sm leading-6 text-slate-300">Create a restricted referral-partner login, a separate referral pipeline, and dedicated legal and Credit Repair submission links. Their submitted matters stay connected to their portal and are assigned to the LegalFlow attorney you choose.</p>
          </div>
          <button onClick={() => { setShowForm(true); setSuccess(null); setError(''); }} className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-400 px-4 py-2.5 text-sm font-bold text-slate-950 transition hover:bg-indigo-300"><Plus className="h-4 w-4" /> Add Referral Partner</button>
        </div>
      </section>
      ...
      {success && <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"><div className="flex gap-3"><CheckCircle2 className="h-5 w-5 shrink-0" /><div><p className="font-semibold">{success.partner?.full_name}’s referral portal is ready.</p><p className="mt-1">{success.email_sent ? 'The login invitation plus private legal and Credit Repair form links were emailed successfully.' : 'The portal was created, but the welcome email could not be confirmed. Check the email address, then share the links manually.'}</p></div></div></div>}

      {showForm && (
        <section className="rounded-2xl border border-indigo-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4 border-b border-slate-200 pb-4">
            <div><h2 className="flex items-center gap-2 text-lg font-bold text-slate-900"><UserPlus className="h-5 w-5 text-indigo-700" /> Create Referral Partner Workspace</h2><p className="mt-1 text-sm text-slate-500">This sends an invitation, creates an isolated partner portal, and provides both legal and Credit Repair intake links. The partner cannot see normal firm cases.</p></div>
          ...
          <form onSubmit={submit} className="mt-5 grid gap-4 sm:grid-cols-2">
            <Field label="Referral Partner Full Name" required><input value={form.full_name} onChange={(event) => update('full_name', event.target.value)} placeholder="Ethan Babb" className="input" /></Field>
            <Field label="Referral Partner Email" required><input value={form.email} onChange={(event) => update('email', event.target.value)} type="email" placeholder="ethan@example.com" className="input" /></Field>
            ...
            <Field label="Private Partner Link Ending"><input value={form.submission_slug} onChange={(event) => update('submission_slug', event.target.value)} placeholder={suggestedSlug(form.full_name) || 'ethan-babb-referrals'} className="input" /><p className="mt-1 text-xs text-slate-500">Leave blank to generate it from the partner’s name. This same ending is used for the partner’s Credit Repair form.</p></Field>
            <div className="sm:col-span-2 flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
              ...
              <button type="submit" disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-700 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-800 disabled:opacity-60">{saving ? <><Loader2 className="h-4 w-4 animate-spin" /> Creating…</> : <><UserPlus className="h-4 w-4" /> Create Portal & Invite</>}</button>
            </div>
          </form>
        </section>
      )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center gap-3 border-b border-slate-200 px-6 py-4"><FolderKanban className="h-5 w-5 text-indigo-700" /><div><h2 className="font-bold text-slate-900">Active Referral Partner Portals</h2><p className="text-sm text-slate-500">Each partner portal has its own pipeline plus private legal and Credit Repair intake links.</p></div></div>
        {!workspaces.length ? <div className="p-10 text-center text-sm text-slate-500">No referral partner portals have been created yet.</div> : <div className="grid gap-4 p-5 md:grid-cols-2">{workspaces.map((partner) => <article key={partner.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-slate-900">{partner.full_name}</h3><p className="mt-1 text-sm text-slate-500">{partner.company || partner.email || 'Referral partner'}</p></div><span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">Active</span></div><div className="mt-4 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600">{referralUrl(partner.submission_slug) || 'Private link is being configured.'}</div>{partner.submission_slug && <div className="mt-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800"><span className="font-semibold">Credit Repair form:</span> {window.location.origin}/credit-repair-lead/{partner.submission_slug}</div>}<ReferralAttorneyFeatureControls partner={partner} onError={setError} /><ReferralPortalCoOwnerControls partner={partner} onError={setError} />
