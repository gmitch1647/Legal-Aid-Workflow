        ...
        {shownClients.length ? shownClients.map((client) => (
          <article key={client.id} className="border-b border-slate-100 p-5 last:border-0">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <Link to={`/attorney/referral-clients/${client.id}`} className="flex min-w-0 items-center gap-3 rounded-lg outline-none transition hover:opacity-80 focus:ring-2 focus:ring-indigo-500"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-sm font-bold text-indigo-700">{client.full_name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span><div className="min-w-0"><h2 className="truncate font-semibold text-slate-900">{client.full_name}</h2><p className="truncate text-sm text-slate-500">{client.email || 'No email recorded'}</p><p className="mt-1 text-xs font-medium text-indigo-700">Open client profile</p></div></Link>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">{client.cases.map((caseItem) => <Link key={caseItem.id} to={`/attorney/referral-cases/${caseItem.id}`} className="rounded-lg bg-slate-50 px-3 py-2.5 text-sm transition hover:bg-indigo-50"><p className="font-medium text-slate-800">{caseItem.plaintiff_name || client.full_name}</p><p className="mt-1 text-xs text-slate-500">{caseItem.status?.replaceAll('_', ' ') || 'Submitted'} · {displayDate(caseItem.created_at)}</p><p className="mt-1 text-xs font-medium text-indigo-700">Open case</p></Link>)}</div>
          </article>
