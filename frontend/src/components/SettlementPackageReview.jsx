...
export default function SettlementPackageReview({ caseRow, onWorkflowChanged, onNotice, onApprovedPackage }) {
  ...
  const [access, setAccess] = useState({ can_review: false, can_submit: false });
  ...
  const loadPackages = useCallback(async () => {
    ...
    try {
      ...
      const [data, permissions] = await Promise.all([getCaseSettlementPackages(caseRow.id), getSettlementPackageAccess()]);
      const rows = Array.isArray(data) ? data : [];
      setPackages(rows);
      setAccess(permissions || { can_review: false, can_submit: false });
      onApprovedPackage?.(permissions?.can_review ? (rows.find((row) => row.status === 'approved') || null) : null);
    }
  ...
  return <>
    <section className="rounded-2xl border border-violet-200 bg-violet-50/60 p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex items-center gap-2 text-violet-800"><ClipboardCheck className="h-5 w-5" /><p className="text-xs font-bold uppercase tracking-wider">Attorney submission & review</p></div><h2 className="mt-1 text-lg font-bold text-violet-950">{access.can_review ? 'Attorney settlement packages awaiting your review' : 'Submit settlement package for owner review'}</h2><p className="mt-1 max-w-3xl text-sm leading-6 text-violet-950/75">{access.can_review ? 'Attorneys submit their settlement agreement and optional credit disclosure to you here. Review the attachments, then approve to stage them in Step 1. Approval never sends anything to the client.' : 'Upload the settlement agreement and optional credit disclosure for the owner to review. The client will not receive anything until the owner approves and sends it.'}</p></div>{access.can_submit && <button type="button" onClick={() => setShowSubmit(true)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-violet-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-800"><Upload className="h-4 w-4" /> Submit package for review</button>}</div>
