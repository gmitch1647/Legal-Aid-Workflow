...
export default function PayoutInformationPanel({ caseId }) {
  ...
  async function releaseToAttorney(item) {
    if (!window.confirm('Release this submitted banking form to the client’s assigned attorney? The attorney will receive a LegalFlow notification but no bank numbers will be sent by email.')) return;
    setReleasingId(item.id); setError('');
    try {
      await releasePayoutInformationToAttorney(item.id);
      await load();
      setSuccess('Payment details were released to the assigned attorney. The attorney can now open an audited secure view from Banking Forms.');
    } catch (err) {
      setError(err.message || 'Could not release payment details to the attorney.');
    } finally {
      setReleasingId('');
    }
  }
  ...
  async function markPaymentSent(event, item) {
    ...
    try {
      await markPayoutPaymentSent(item.id, {
        payment_amount: amount,
        payment_sent_at: form.payment_sent_at ? new Date(`${form.payment_sent_at}T12:00:00Z`).toISOString() : null,
      ...
      setError(err.message || 'Could not record the client payment.');
    } finally {
      setMarkingPaymentId('');
    }
  }
  ...
  async function revokeAttorneyAccess(item) {
    ...
    try {
      ...
      await load();
      setSuccess('Attorney payment-detail access was revoked.');
    } catch (err) {
      setError(err.message || 'Could not revoke attorney access.');
    } finally {
      setRevokingId('');
    }
  }
        ...
        {loading ? <p className="flex items-center gap-2 text-sm text-slate-500"><Loader2 className="h-4 w-4 animate-spin" />Loading payout-information requests…</p> : requests.length === 0 ? <p className="rounded-lg border border-dashed border-emerald-200 bg-white/80 px-3 py-4 text-sm text-slate-500">No payout-information requests have been sent for this case.</p> : requests.map((item) => (
          <div key={item.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-emerald-100 bg-white p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-sm text-slate-500">{item.status === 'completed' && item.submission ? `${item.submission.account_ownership ? `${item.submission.account_ownership === 'business' ? 'Business' : 'Personal'} · ` : ''}${item.submission.account_type === 'checking' ? 'Checking' : 'Savings'} account ending in ${item.submission.account_number_last4} · submitted ${item.submission.submitted_at ? new Date(item.submission.submitted_at).toLocaleDateString() : 'today'}` : item.due_date ? `Requested by ${new Date(`${item.due_date}T00:00:00`).toLocaleDateString()}` : 'Waiting for the client to complete the secure form.'}</p>
            </div>
